...
| `java/scripts/smoke_launcher.sh` | P26-004: quoting/detach/`--foreground`/fail→log (stub, no JavaFX) |
...
JaCoCo gate ≥80% instruction coverage (JavaFX UI and `ProcessRouteProbe.trace` excluded — tested via parser/unit separately; IPv6 `HopDisplay`/GeoIP overrides in bundle).

Trace fixtures: `java/src/test/resources/trace/` — v4 regression: `ProcessRouteProbeTest.v4FixturesRemainGreen`.

LOG sink contract (P16-072): `SyslogGelfContractTest` + `TelemetryLogFieldFixture` — mock TCP syslog/GELF, shared event fields.
