...
| Field | Value |
|-------|-------|
| **Branch** | `main` — stable snapshot after merge; `beta` — development (phase **36** — GeoIP / IP metadata). Both: Java Pro (P9–P19) + Python after merge |
...
| Field | Value |
|------|----------|
| **Current task** | **P36-001** |
| **Phase** | 36 — GeoIP / IP metadata (offline enrichment) |
| **DoD (short)** | Java-only, offline-only, no probe blocking; country hints positioning |
| **Branch** | `beta` |
...
| 111 | **P26-009** | [x] | ADR_HARDENING + LIVING_SPEC/CHECKLIST + phase close |
...
| 147 | **P33-004** | [x] | `poll_result` tri-state (error ≠ downtime) |
| 148 | **P33-005** | [x] | Latency baseline reset on route/mode change |
...
**Queue status:** **NEXT = P36-001** (phase 36; [pingui-geoip.md](pingui-geoip.md)).
...
| ID | Task | Files | DoD |
|----|------|-------|-----|
| **P13-001** | [x] ADR: `probe_mode: trace \| mtr \| ping_only` | `docs/ADR_PROBE_MODES.md` | MTR = continuous per-hop, not full trace each cycle |
...
## Phase 19 — Production hardening (`beta`, P0–P2)

**Goal:** close risks after `main` merge (PR #10): release hygiene, cross-platform CI gate, more meaningful OS-critical coverage, transitional probe-mode debt removal, first `MonitorService` decomposition step.

**Context:** repeat architecture review of `main` @ `176a9d3` (2026-07): JaCoCo 80% with large exclusions; Windows job non-blocking; `version` vs `appVersion`; dual `pingOnly`/`probeModes`; `MonitorService` god object; `postgresql` on fat classpath.
...
| **P19-005** | [x] MonitorService slice: HostRegistry | `monitor/HostRegistry.java`, `MonitorService` | Extract host list, enabled, probe mode, interval override, rename/remove; `MonitorService` delegates; behavior unchanged; registry unit tests |
| **P19-006** | [x] PostgreSQL driver scope (P2) | `build.gradle.kts`, persistence wiring docs | `postgresql` not in unconditional `implementation` (optional/`runtimeOnly`/feature flag); desktop install without PG does not bundle driver; LIVING_SPEC/DEPLOYMENT note |
...
| **P20-006** | [x] Keyboard accelerators | `AppAccelerators`, `MainController`, `AppMenuDialogs` | Hotkeys: Save, Add host, Help(F1); document in Help; do not break TextField focus |
| **P20-007** | [x] Empty states | `EmptyStateHints`, `RouteHistoryPresenter`, `ViewModeController` | Hints when no SQLite / empty history / Simple without log; UK/EN |
...
| **P26-009** | [x] Phase close | `ADR_HARDENING.md`, LIVING_SPEC, CHECKLIST, ROADMAP | NEXT→**P28-001**; phase 26 closed |
...
## Phase 28 — Runtime hardening follow-up (`beta`, P1)

**Context:** Audit `pimgui-5.md` (main @ `3de5cf85`, 2026-08-18). JaCoCo package gates are already **P26-007**. Java SQLite exact-version reject is in P27. Remaining: telemetry hang isolation, poll queue, Python forward-compat.
...
**Queue:** after P29; **NEXT = DONE** (phase 30 closed).
...
| **P30-005** | [x] Rollup + retention | `SessionDatabase`, `PollResultRetentionJob`, CLI `--poll-retention` | schema v12; `metric_rollup` + bounded retention (7d raw / 5m→90d / 1h beyond) |
| **P30-006** | [x] Export reliability | `SessionDatabase.readOnly`, export, CLI | RO connection for export/dump; `--integrity-check`; backup note; `busy_timeout` already in P30-001 |

**Out of scope:** Postgres desktop; ORM; full hop/ASN/DNS IP normalization; silent migrate.

---

## Phase 31 — GUI information hierarchy (`beta`, P1)
...
**Queue:** after P32; **NEXT = DONE** (P33-001…008 [x]; phase 33 closed).
...
| **P33-002** | [x] Projection semantics | `SessionStore`, classifiers, telemetry | phase/`targetSampled`/`routeChanged` in projection; partial discovery ≠ route change; endpoint only when target sampled |
| **P33-003** | [x] SessionStore + bounded writers | `SessionStore`, persistence, Influx/TS | Fast in-memory path; SQLite/TS off FX/probe; immutable API snapshot; drop/degraded counter |
| **P33-004** | [x] `poll_result` tri-state | `MonitorService`, `PollResultEffects` | Probe/internal error → `target_sampled=false`, `reachable=null` (not downtime) |
| **P33-005** | [x] Latency baseline reset | `MonitorService` / alert EWMA | Reset on confirmed route change and `setHostProbeMode` |
| **P33-006** | [x] Webhook lifecycle | `WebhookTelemetrySink`, alert dispatcher | Bounded queue + rejected counter; closeable dispatcher on replace/stop |
| **P33-007** | [x] DB migrate + chunked retention | `SchemaManager`, retention job | v12→v14 (or offline CLI); chunked retention |
| **P33-008** | [x] Docs / branch sync | README, ROADMAP, indexes | Align phases/NEXT/`main`≡`beta`; archival note for P32 audit |
...
## Phase 34 — Correctness follow-up: route / target / persistence (`beta`, P0)
...
| **P34-002** | [x] MTR target-unknown / exhaustion | `MtrProbe`, `MtrProbeState` | `targetHop` only after real match; `TARGET_UNKNOWN`/`INCOMPLETE`; bounded rediscovery; no `target_sampled` until identified; maxHops exhausted test |
...
**Queue:** after P34; **NEXT = DONE** (phase 35 closed; superseded by P36).
...
| **P35-006** | [x] MTR invalidate on DNS change | `MonitorService.applyDnsControlEvent`, `RoutePoller.invalidateOnDnsAddressChange` | Address-set change clears targetIp/candidate/latency; NEXT→P35-007 |
...
| **P35-008** | [x] Stuck-writer fault test | SessionPersistenceWriter | close must not caller-drain while stuck worker alive; NEXT→P35-009 |
| **P35-009** | [x] Route signature ≡ hops_json | RouteSignature | One hop-indexed source; NEXT→P35-010 |
| **P35-010** | [x] Python lifecycle harden | `src/pingui/` | join/drain without closing DB under live worker; NEXT→DONE |

**Out of scope:** new protocols; ORM; large GUI surfaces; Python feature parity; silent `.db` delete.

## Phase 36 — GeoIP / IP metadata: offline enrichment (`beta`, P0)
...
**Queue:** after P35; **NEXT = P36-001**.

| ID | Task | Files | DoD |
|----|------|-------|-----|
| **P36-001** | [ ] Contract and boundaries | docs, `GeoCountry` positioning | Java-only, offline-only, no probe blocking; “country hints” |
...
**Out of scope:** network GeoIP API; heavy Java map; GeoIP in `poll_result`; Python feature parity; MMDB in Git.
...
**Backlog (historical sprint line):** M/B roadmap closed; **IPv6 — Phase 9**; **Python NOC — Phase PY**; **Pro — Phases 10–19**; **Phase 20 GUI UX**. Authoritative linear queue — **[NEXT](#next--single-source-of-truth)** only (currently **P36-001**).
