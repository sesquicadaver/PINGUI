...
**Runtime attach (P16-090 ✅):** `TelemetryAttachment` — shared path for `DaemonRunner` and JavaFX `MainController` (install sinks → bus → `MonitorService.setTelemetryBus`). Close order: monitor → attachment → session store. GUI settings dialog — P16-091+.
