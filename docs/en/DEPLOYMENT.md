> **Language:** [Ukrainian](../DEPLOYMENT.md) · English
...
**IPv4 trace/raw ICMP; dual-stack config (V6-S1):** YAML and session accept IPv6 literals (RFC 5952, e.g. `2001:db8::1` or `[::1]`). Subprocess trace and raw ICMP for v6 — phase 9.2+ ([ROADMAP.md](ROADMAP.md)); probe remains IPv4-oriented for now.

## OS recommendation

| Platform | For daily use | Comment |
|----------|---------------|---------|
| **Linux** | ✅ **Recommended** | `traceroute -q 1` — fast; Expert ping; raw ICMP |
| **macOS** | ✅ Good | Fast trace; Expert ping unavailable |
| **Windows** | ⚠ Limited | **Slow `tracert`**: 3 probes per hop, ~4 s wait per probe → full trace 1–4+ min per target at 20 hops. Expert ping unavailable |

**Windows is not the best choice** for continuous route monitoring with short `interval` (e.g. 1 s): the next cycle often starts before the previous trace completes.

**Mitigations on Windows:**

- In GUI: **Ping only** checkbox on the host (RTT to target only, no hops).
- In YAML: `ping_only: true` or `interval: 30` (or higher) for trace mode.
- Expect 1–4 minute delay on first full trace to a remote target.
...
Example: `java/config/hosts.example.yaml`. v2 format:
...
| Symptom | Solution |
|---------|----------|
| Trace on Windows «hangs» / very slow | Normal for `tracert`; enable **Ping only** or increase `interval` |
...
Tests, CI, Python edition — **`beta`** branch.
