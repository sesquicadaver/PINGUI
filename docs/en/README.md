...
On **`main`** and **`beta`** — full package (Java Pro + Python) after merge; **`beta`** is the development branch, **`main`** is the last stable snapshot. The linear ROADMAP queue is currently **DONE** (see [ROADMAP.md § NEXT](ROADMAP.md#next--single-source-of-truth)). Branches: [README.en.md](../README.en.md).
...
| [ROADMAP.md](ROADMAP.md) | Developer | Official plan: phases 0–17 (Pro/NOC + telemetry + Expert MTU); NEXT=DONE |
