...
5. **Expert ping** — **Expert** checkbox → **Exten.** → presets **MTU probe / DF / DSCP / Burst** from `ping_presets.yaml` (AF `-4`/`-6` is kept). Each preset only fills `ping(8)` flags and shows summary/expect in the dialog; it is **not** an automatic MTU sweep (wizard — ROADMAP P17-020/021).
