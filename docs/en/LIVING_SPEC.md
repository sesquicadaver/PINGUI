...
| GeoIP country hints (pre-P36) | `GeoCountry` | `GeoCountryTest` (longest-prefix, LAN/IPv6, invalid YAML, 0.0.0.0/0); phase 36 → [pingui-geoip.md](pingui-geoip.md) |
...
| MTR per-hop probe (P13-010 / P33-001 / P34-001) | `MtrProbe`, `MtrProbeState` (`targetHop`/`monitoringSpan`), `RoutePoller.pollHostMtr`, `CandidateRouteFsm` | `MtrProbeTest` (timeout span / target recovery), `RoutePollerTest.pollHostMtrConfirmsRouteChangeOnlyAfterTarget` |
...
| SQLite reopen/corrupt (P26-003 / P27-001) | `SessionDatabase` reject legacy schema; corrupt/truncated open | `SessionDatabaseHardeningTest`; `SessionStorePersistenceTest.appendPingSamplesAfterDbReopen` |
...
| Sqlite telemetry sink (P16-020 / P27-001) | `SqliteTelemetrySink`; schema v5 columns SSOT | `SqliteTelemetrySinkTest` |
...
| Webhook as TelemetrySink (P16-050 / P33-006) | `WebhookTelemetrySink` (bounded queue + `rejectedCount`); `WebhookAlertDispatcher`/`AlertDispatcher` closeable | `WebhookTelemetrySinkTest`; `WebhookAlertDispatcherTest`; `AlertDispatchersTest`; `PollResultEffectsTest.setAlertDispatcherClosesPrevious*` |
...
| GeoIP / IP metadata (P36) | NEXT=**P36-001**; queue P36-001…012 | [pingui-geoip.md](pingui-geoip.md) |
...
| MTR invalidate on DNS change (P35-006) | `DnsControlEvent.isAddressSetChange`, `MonitorService.invalidateHostAfterDnsAddressChange` | `DnsAddressChangeInvalidationTest.pollPathDnsChangeInvalidatesMtrTarget`, `applyDnsControlEventChangeInvalidatesButOkDoesNot` |
| Unified SQLite persistence (P35-007) | `PollPersistenceBatch`, `SessionPersistenceWriter.offerPollHistory`, `PollResultEffects.recordCompletedPoll` | `PollHistoryPipelineTest.offerPollHistoryWritesRouteAndPollResultInOneTransaction`, `recordCompletedPollDoesNotBlockCallerOnSlowApplier` |
| Route identity FSM (P34-001) | `RouteIdentity`, `CandidateRouteFsm`, `RouteChangeDetector`, `RoutePoller`, `RouteSignature` | `RouteIdentityTest`, `CandidateRouteFsmTest`, `RouteChangeDetectorTest`, `RoutePollerTest.pollHostMtrConfirmsRouteChangeOnlyAfterTarget`, `SessionDatabaseRouteTest.transientTimeoutDoesNotCreateNewRouteRow`, `RouteSignatureTest.observeRouteKeepsSignatureAlignedWithHopsJson` |
...
| Persistence lanes (P34-004) | `SessionPersistenceWriter` control/coalesce/telemetry | `SessionPersistenceWriterTest.telemetryOverflowDoesNotDropDelete`, `telemetryOverflowDoesNotDropRename`, `coalescesSaveHostToLatestSnapshot`, `closeStopsWorkerBeforeReturning`, `closeDoesNotCallerDrainWhileStuckWorkerAlive` |
| Immutable CompletedPoll (P34-005) | `CompletedPoll`, `MonitorLifecycle`, GUI/daemon parity | `CompletedPollTest.guiAndDaemonListenersShareSamePollResultWhenUsingCompletedPoll`, `recordCompletedPollDoesNotTouchSessionStore` |
...
| Python compatibility (P34-009 / P35-010) | `src/pingui/` bugfix-only lock; idempotent close; no DB close under live worker | `test_version.py`, `test_session_db.py`, `test_daemon_runner.py`, `test_monitor_telemetry.py` (stuck close) |
...
| DNS control (P29-004) | `DnsControl`, `DnsControlTracker`, `PersistenceEventWriter.writeDnsChange`, `MonitorService.observeDnsControl` | `DnsControlTest`, `PersistenceEventWriterTest.writesDnsChange*`, `IncidentTimelineBuilderTest.includesDnsChange*` |
| Alert silence (P29-003) | `AlertSilenceConfig`, `PollResultEffects`, `AlertsSettingsDialog`, YAML `alerts.silence` | `AlertSilenceConfigTest`, `ProfilesConfigTest.loadAndSaveAlertSilence`, `PollResultEffectsTest.silence*` |
| Incident timeline (P29-002) | `IncidentTimelineBuilder`, `listHostEvents`, `PROBLEM_ACK`, Extended history | `IncidentTimelineBuilderTest`, `RouteHistoryPresenterTest`, `PersistenceEventWriterTest` |
...
| SQLite session (P11… / P32-004 / P33-007) | `SessionDatabase` v14 (migrate 12→13→14); chunked `PollResultRetentionJob` | `SessionDatabaseMetricRollupTest`, `PollResultRetentionJobTest` |
...
Module → test matrix for the Python edition (**legacy / bugfix-only**, P32-008 / P34-009). Do not add features / schema evolution here — bugfix and security only; Java is canonical.
