...
| SQLite schema evolution (P30) | schema v8+: stable `host_session.id` / `host_id`; incident/poll/route/rollup | [ADR_SESSION_SCHEMA.md](ADR_SESSION_SCHEMA.md); delete/recreate; P30-001 [x]; NEXT=P30-002 |
...
| SQLite session (P11-010 / P27-003 / P30-001) | `SessionDatabase` schema v8 (`host_session.id` + `host_id` FKs); address-keyed API | `SessionDatabaseTest`, `SessionDatabaseHardeningTest` |
