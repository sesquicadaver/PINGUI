...
| SQLite schema evolution (P30) | schema v8+: stable `host_session.id` / `host_id`; `incident` (v9); poll/route/rollup next | [ADR_SESSION_SCHEMA.md](ADR_SESSION_SCHEMA.md); delete/recreate; P30-002 [x]; NEXT=P30-003 |
...
| SQLite session (P11-010 / P27-003 / P30-001 / P30-002) | `SessionDatabase` schema v9 (`host_id` + `incident`); address-keyed API | `SessionDatabaseTest`, `SessionDatabaseIncidentTest`, `SessionDatabaseHardeningTest` |
