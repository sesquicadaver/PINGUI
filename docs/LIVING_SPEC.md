...
| SQLite schema evolution (P30) | schema v8–v12: host id; `incident`; `poll_result`; deduped `route`; `metric_rollup` + retention | [ADR_SESSION_SCHEMA.md](ADR_SESSION_SCHEMA.md); delete/recreate; P30-005 [x]; NEXT=P30-006 |
...
| SQLite session (P11… / P30-005) | `SessionDatabase` schema v12 (+ `metric_rollup`); address-keyed API | `SessionDatabaseMetricRollupTest`, `PollResultRetentionJobTest`, `SessionDatabaseHardeningTest` |
