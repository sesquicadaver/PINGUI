...
**Схема:** Java `SCHEMA_VERSION = 14` (P32-004: additive `metric_rollup`). **v12→v13→v14** мігрує на відкритті; `<v12` — видалити файл і створити наново ([ADR_SESSION_SCHEMA.md](ADR_SESSION_SCHEMA.md)). Probe-error рядки з legacy backfill виправляються автоматично (P34-008) або через:
