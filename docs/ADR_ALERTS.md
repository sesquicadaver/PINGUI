...
Python містить референсну реалізацію (`alert_dispatcher.py`, `alert_rate_limiter.py`, `desktop_notifier.py`). Java — повний alert pipeline (фаза 10, P10-010…P10-050) на `main` і `beta` після merge.
...
- Збагачення payload (ASN, geo, diff summary) — **P36-009:** optional `geo_diff` / `asn_diff` на webhook; `detail_json` у SQLite; base ADR fields без змін.
