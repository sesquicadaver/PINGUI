...
**SQLite samples (P16-020 / P27-001):** `SqliteTelemetrySink` зберігає samples/events у schema v5 (типізовані колонки + `labels_json`; default off / retention P16-022). Це **локальний архів**, не заміна Prometheus scrape і не Grafana TS datasource за замовчуванням. Межа з §2 («не писати hop-RTT у SQLite як Prometheus replacement») лишається в силі для P15 і для операторських dashboards.
