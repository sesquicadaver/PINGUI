...
Schema **v4** (`SCHEMA_VERSION`): JSON hops/routes/`hop_stats` + `persistence_event` + telemetry tables. **P28-003:** `version != SCHEMA_VERSION` → `SessionDatabaseError` (без silent migrate; видалити `.db` і створити наново). Java session schema — окремо **v14** (P32-004: additive `metric_rollup` + atomic retention; migrate 13→14; також `probe_outcome` / RO export / `route` / `incident`).
...
Quality-правило окремо від `endpoint_down`: `rtt ≥ multiplier × AVG` (default 2.0; AVG = EWMA α=0.2), `fail_after=3` поспіль без часового вікна; YAML `alerts.rules.latency_high`; GUI «Сповіщення…» з ETA ≈ fail_after×interval; badge/ack через `HostProblemSummary`; SQLite `persistence_event.latency_high`.
...
### Java: DnsControl (P29-004 / P35-005 / P35-006)

Forward DNS for hostname targets: address set (v4/v6), resolve latency, distinct `dns_change` events (`ok` / `change` / `nxdomain` / `timeout` / `servfail`). `DnsControlTracker` via bounded/coalesced `DnsControlDispatcher` (≤1 pending/host); outer queue in `/ops` `dns_control` + Prometheus `pingui_dns_control_*`. Confirmed `change` invalidates MTR targetIp, candidate route, and latency baseline. Persistence only — not an auto-incident.
