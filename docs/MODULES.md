...
Schema **v4** (`SCHEMA_VERSION`): JSON hops/routes/`hop_stats` + `persistence_event` + telemetry tables. **P28-003:** `version != SCHEMA_VERSION` → `SessionDatabaseError` (без silent migrate; видалити `.db` і створити наново). Java session schema — окремо **v12** (P30-005…006: `metric_rollup`, RO export, integrity_check; також `route` / `poll_result` / `incident` / `host_id`).
