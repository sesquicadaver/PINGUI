...
| Поле | Значення |
|------|----------|
| **Поточна задача** | **P33-006** |
| **Фаза** | 33 — Correctness (MTR / projection / side-effects) |
| **DoD (коротко)** | Webhook bounded queue + closeable dispatcher |
| **Гілка** | `beta` |
...
**Стан черги:** **NEXT = P33-006** (фаза 33; [pingui-correctness.md](pingui-correctness.md)).
...
**Backlog (історичний sprint-рядок):** M/B roadmap закрито; **IPv6 — Фаза 9**; **Python NOC — Фаза PY**; **Pro — Фази 10–19**; **Фаза 20 GUI UX**. Актуальна лінійна черга — лише секція **[NEXT](#next--єдине-джерело-правди)** (зараз **P33-006**).
