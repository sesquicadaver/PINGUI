...
| Поле | Значення |
|------|----------|
| **Поточна задача** | **[P16-093](#фаза-16--телеметрія-метрики-мережі--log-server-beta--main-p0p1)** |
| **Фаза** | 16 — Телеметрія (GUI) |
| **DoD (коротко)** | Python: wire YAML telemetry або явно «Java/daemon only» |
| **Гілка** | `beta` |
...
**Контекст:** sinks з YAML/`--telemetry-*` працюють у `DaemonRunner`; JavaFX GUI зберігає `telemetry:` при Save, але **не** ставить bus → події з desktop не йдуть у sinks.
