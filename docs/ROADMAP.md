...
| Поле | Значення |
|------|----------|
| **Гілка** | `main` — стабільний зріз після merge; `beta` — розробка (лінійна черга **P34**). Обидві: Java Pro (P9–P19) + Python після merge |
...
| Поле | Значення |
|------|----------|
| **Поточна задача** | **P34-001** |
| **Фаза** | 34 — Correctness follow-up (route / target / persistence) |
| **DoD (коротко)** | Route identity + candidate-route FSM для TRACE/MTR |
| **Гілка** | `beta` |
...
| 54 | **P19-001** | [x] | Єдине джерело версії (`appVersion` ← `version`) |
...
| 109 | **P26-007** | [x] | JaCoCo package-level thresholds (не один bundle) |
...
| 148 | **P33-005** | [x] | Latency baseline reset на route/mode change |
...
**Стан черги:** **NEXT = P34-001** (фаза 34; [pingui-route-persistence.md](pingui-route-persistence.md)).
...
| **P15-020** | [x] Java parity: InfluxDB/Timescale writer (як Python B-05) | `persistence/timeseries/` | Config parity з Python |
...
| **P16-013** | [x] Wire MonitorService → bus (RTT, loss, route_change, probe_error) | `MonitorService`, `worker.py` | Не блокує poll loop |
| **P16-014** | [x] Метрики: `trace_duration_ms`, `target_reachable` | `telemetry/MetricNames.java` | Labels: profile, probe_mode, edition |
...
| **P16-023** | [x] Export з local store: `--telemetry-dump` (CSV/JSON) | `export/TelemetryDump.java` | Cron-friendly one-shot |
...
| ID | Задача | Файли | DoD |
|----|--------|-------|-----|
| **P16-060** | [x] CONFIGURATION § telemetry | `docs/CONFIGURATION.md`, `docs/en/CONFIGURATION.md` | Повна таблиця полів |
...
| ID | Задача | Файли | DoD |
|----|--------|-------|-----|
| **P19-001** | [x] Єдине джерело версії | `java/build.gradle.kts`, `java/README.md`, `docs/DEPLOYMENT.md` | `jpackage --app-version` береться з `version` (без окремого `0.1.0`); `build.properties`/`About` узгоджені; CHANGELOG note |
...
| **P26-007** | [x] Package JaCoCo thresholds | `java/build.gradle.kts` | PACKAGE minima: config/probe/monitor ≥85%, telemetry ≥80%, persistence ≥75%; BUNDLE ≥80%; UI `io/pingui/ui/**` exclusion явний |
| **P26-008** | [x] Latency baseline evolution | `AlertRuleEngine`, `LatencyHighRuleConfig`, ADR_ALERT_RULES, i18n | EWMA α=0.2; тест vs unbounded mean; Help/Settings ETA ≈ fail_after×interval |
| **P26-009** | [x] Phase close | `ADR_HARDENING.md`, LIVING_SPEC, CHECKLIST, ROADMAP | NEXT→**P28-001**; фаза 26 closed |
...
| **P28-003** | [x] Python schema version gate | `session_db.py`, unit tests | `SessionDatabaseError` якщо `version != 4`; без silent migrate; tests older/newer |
...
**Черга:** після P28; **NEXT** перейшов на фазу 30 (див. вище).
...
## Фаза 30 — SQLite schema evolution (`beta`, P1)
...
**Черга:** після P29; **NEXT = DONE** (фаза 30 закрита).
...
| **P30-006** | [x] Export reliability | `SessionDatabase.readOnly`, export, CLI | RO connection для export/dump; `--integrity-check`; backup note; `busy_timeout` уже в P30-001 |

**Поза scope:** Postgres desktop; ORM; повна нормалізація hops/ASN/DNS IP; silent migrate.
...
**Черга:** після P31; **NEXT** перейшов на фазу 33 (P32-001…008 [x]; фаза 32 закрита).
...
| **P33-004** | [x] `poll_result` tri-state | `MonitorService`, `PollResultEffects` | Probe/internal error → `target_sampled=false`, `reachable=null` (не downtime) |
| **P33-005** | [x] Latency baseline reset | `MonitorService` / alert EWMA | Скидання на confirmed route change і `setHostProbeMode` |
| **P33-006** | [x] Webhook lifecycle | `WebhookTelemetrySink`, alert dispatcher | Bounded queue + rejected counter; closeable dispatcher при заміні/stop |
| **P33-007** | [x] DB migrate + chunked retention | `SchemaManager`, retention job | v12→v14 (або offline CLI); retention порціями |
| **P33-008** | [x] Docs / branch sync | README, ROADMAP, indexes | Узгодити фази/NEXT/`main`≡`beta`; архівний note для P32 audit |

**Поза scope:** повний candidate-route FSM beyond P33-001 span fix; ORM; silent delete `.db`.

---


## Фаза 34 — Correctness follow-up: route / target / persistence (`beta`, P0)
...
**Черга:** після P33; **NEXT = P34-001**.

| ID | Задача | Файли | DoD |
|----|--------|-------|-----|
| **P34-001** | [ ] Route identity + candidate FSM | `RouteChangeDetector`, `MtrProbe`, `RoutePoller`, `RouteSignature` | Hop-indexed `(hop, ip\|timeout)`; active/candidate; confirm до target; transient `*` ≠ deduped route; один event на фізичну зміну |
| **P34-002** | [ ] MTR target-unknown / exhaustion | `MtrProbe`, `MtrProbeState` | `targetHop` лише після real match; `TARGET_UNKNOWN`/`INCOMPLETE`; bounded rediscovery; без `target_sampled` до ідентифікації; тест maxHops exhausted |
| **P34-003** | [ ] Current endpoint vs history | `HostNetworkStateClassifier`, UI states | Current TIMEOUT → DOWN/PENDING; session-lifetime loss не маскує актуальний timeout; тест timeout після успіхів |
...
**Backlog (історичний sprint-рядок):** M/B roadmap закрито; **IPv6 — Фаза 9**; **Python NOC — Фаза PY**; **Pro — Фази 10–19**; **Фаза 20 GUI UX**. Актуальна лінійна черга — лише секція **[NEXT](#next--єдине-джерело-правди)** (зараз **P34-001**).
