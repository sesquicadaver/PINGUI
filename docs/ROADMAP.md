...
| Поле | Значення |
|------|----------|
| **Поточна задача** | **P35-002** |
| **Фаза** | 35 — Unattended NOC (probe identity / loss / persistence) |
| **DoD (коротко)** | `probedHop<1` ≠ FULL; rediscovery; без downtime з TARGET_UNKNOWN |
| **Гілка** | `beta` |
...
**Стан черги:** **NEXT = P35-002** (фаза 35; [pingui-unattended.md](pingui-unattended.md)).
...
**Черга:** після P33; **NEXT = DONE** (P34-001…010 [x]; фаза 34 закрита).
...
| **P34-010** | [x] Soak/docs sync | tests, README, ROADMAP | Regression matrix з аудиту; NEXT/`main`≡`beta` |

**Поза scope:** нові протоколи; ORM; великі GUI-екрани; silent delete `.db`.

---

## Фаза 35 — Unattended NOC: probe identity / loss / persistence (`beta`, P0)
...
| **P35-002** | [ ] MTR TARGET_UNKNOWN scope + rediscovery | `RoutePoller`, `MtrProbe` | `probedHop<1` ≠ `PollSampleScope.FULL`; periodic rediscovery; без downtime з UNKNOWN |
| **P35-003** | [ ] Loss window semantics | `PollResultEffects`, hop stats | Fixed/sliding window **або** loss=NULL у single-packet |
...
**Backlog (історичний sprint-рядок):** M/B roadmap закрито; **IPv6 — Фаза 9**; **Python NOC — Фаза PY**; **Pro — Фази 10–19**; **Фаза 20 GUI UX**. Актуальна лінійна черга — лише секція **[NEXT](#next--єдине-джерело-правди)** (зараз **P35-002**).
