...
- **P35 NEXT activated:** ROADMAP **NEXT = P35-001** → **P35-007** після P35-001…006; черга P35-001…010; [pingui-unattended.md](docs/pingui-unattended.md).
...
- **P31-003 — host inspector:** компактна панель у Extended (metrics/last poll/route/problem + copy/ack/diagnostics); NEXT→**P31-004**.
- **P31-002 — endpoint vs route states:** незалежні Endpoint (UP/DEGRADED/DOWN/UNKNOWN) і Route (STABLE/CHANGED/INCOMPLETE/NOT TRACED); PING_ONLY/TCP → NOT TRACED без помилкового ✕; NEXT→**P31-003**.
- **P31-001 — unified host row:** колонки `[state] name RTT loss mode [problem]`; фіксована висота 36px; poll/RTT/tags у tooltip; NEXT→**P31-002**.
...
- **P27-003 — normalized host_session (schema v7):** `session_route_hop` / `session_ping_sample` / `session_hop_stats` / `session_hop_rtt_sample`; `load`/`save` API без змін.
- **P27-002 — typed persistence_event (schema v6):** колонки `state`/`message`/`old_ips_json`/`new_ips_json`/`detail_json`; без `payload_json`; `payloadJson()` лише для UI rebuild.
