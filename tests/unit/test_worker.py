...
def test_worker_add_host_enabled() -> None:
    worker = LightweightMonitorWorker([], interval_seconds=60.0)
    added = worker.add_host("1.1.1.1", enabled=True)
    assert added == "1.1.1.1"
    assert worker.enabled_hosts() == ["1.1.1.1"]


def test_worker_can_add_host_at_limit() -> None:
    hosts = [f"10.0.0.{i}" for i in range(10)]
    worker = LightweightMonitorWorker(hosts, interval_seconds=60.0)
    assert not worker.can_add_host()
    with pytest.raises(ConfigError, match="Maximum"):
        worker.add_host("9.9.9.9")


def test_worker_set_enabled_unknown_host() -> None:
    worker = LightweightMonitorWorker(["8.8.8.8"], interval_seconds=60.0)
    with pytest.raises(ConfigError, match="Unknown"):
        worker.set_host_enabled("9.9.9.9", True)


def test_worker_set_enabled_toggle() -> None:
    worker = LightweightMonitorWorker(["8.8.8.8", "1.1.1.1"], interval_seconds=60.0)
    worker.set_host_enabled("8.8.8.8", True)
    worker.set_host_enabled("8.8.8.8", False)
    assert worker.enabled_hosts() == []


def test_worker_remove_unknown_host() -> None:
    worker = LightweightMonitorWorker(["8.8.8.8"], interval_seconds=60.0)
    with pytest.raises(ConfigError, match="Unknown"):
        worker.remove_host("9.9.9.9")


def test_worker_rename_unknown_host() -> None:
    worker = LightweightMonitorWorker(["8.8.8.8"], interval_seconds=60.0)
    with pytest.raises(ConfigError, match="Unknown"):
        worker.rename_host("9.9.9.9", "1.1.1.1")
