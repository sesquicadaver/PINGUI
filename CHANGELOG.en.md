...
- **P36-007 — GeoIP bootstrap/CLI:** `--geoip-db` / `--geoip-asn-db` + `IpMetadataBootstrap` (fail-fast on explicit broken MMDB; YAML-only without MMDB); `--asn-timeout-ms` deprecated; NEXT→**P36-008**.
...
- **P33-008 — docs / branch sync + phase close:** phases/NEXT/`main`≡`beta` aligned; archival notes for P32+P33; ROADMAP **NEXT=DONE** (phase 33 closed).
...
- **P27-001 — telemetry schema v5:** `telemetry_sample`/`telemetry_event` without `payload_json`; `labels_json` (+ `old_ips_json`/`new_ips_json`); list/dump from columns; legacy schema → PersistenceException.
