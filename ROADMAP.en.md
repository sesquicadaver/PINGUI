...
| Field | Value |
|------|----------|
| **Current task** | **[P34-008](docs/en/ROADMAP.md#next--single-source-of-truth)** |
| **Rule** | if not `DONE` — `/autopilot` = this ID; if `DONE` — stop / wait for an explicit new ID. **Do not ask** “which item?”. |

Full linear queue: [docs/en/ROADMAP.md — Execution queue](docs/en/ROADMAP.md#execution-queue-linear) (phase 34; NEXT=**P34-008**).
