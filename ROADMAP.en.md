...
| Field | Value |
|------|----------|
| **Current task** | **[P36-002](docs/en/ROADMAP.md#next--single-source-of-truth)** |
| **Rule** | if not `DONE` — `/autopilot` = this ID; if `DONE` — stop / wait for an explicit new ID. **Do not ask** “which item?”. |

Full linear queue: [docs/en/ROADMAP.md — Execution queue](docs/en/ROADMAP.md#execution-queue-linear) (phase 36; NEXT=**P36-002**).
...
| **36** | GeoIP / IP metadata (offline enrichment) | 🔄 **P36-002** |
